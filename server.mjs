...
${connected ? `<p class="status ok">Conta TikTok conectada. O próximo passo é consultar as permissões disponíveis e publicar uma imagem.</p>
<form method="post" action="/publish"><label for="image">URL pública verificada da imagem</label><input id="image" name="image" type="url" placeholder="https://ramongarcia30.github.io/tiktok-openclaw/imagem.jpg" required><p class="muted">A URL precisa ser HTTPS, pública e estar dentro do prefixo verificado no TikTok.</p><label for="title">Título</label><input id="title" name="title" maxlength="90" placeholder="Título da publicação"><label for="description">Descrição</label><input id="description" name="description" maxlength="4000" placeholder="#arte #tiktok"><label for="privacy">Privacidade</label><select id="privacy" name="privacy"><option value="PUBLIC_TO_EVERYONE">Público</option><option value="SELF_ONLY">Somente eu</option></select><button type="submit">Publicar imagem diretamente</button></form>` : `<p>O login é feito pelo TikTok. O aplicativo só solicita as permissões configuradas e armazena o token localmente.</p><a href="/auth/tiktok"><button type="button">Conectar com TikTok</button></a>`}
...
async function route(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && requestUrl.pathname === '/publish') {
      ...
      const publish = await tiktok('/v2/post/publish/content/init/', token.access_token, { post_info: { title: form.get('title') || undefined, description: form.get('description') || undefined, privacy_level: privacy, disable_comment: false, auto_add_music: false, brand_content_toggle: false, brand_organic_toggle: false }, source_info: { source: 'PULL_FROM_URL', photo_cover_index: 0, photo_images: [image] }, post_mode: 'DIRECT_POST', media_type: 'PHOTO' });
